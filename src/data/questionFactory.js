...
const levelBand=level=>{if(/^P[1-3]$/.test(level))return{max:99};if(/^P[4-6]$/.test(level))return{max:999};if(/^JSS/.test(level))return{max:9999};return{max:100000}};
...
function make(topic,level,index){const f=familyFor(topic),n=index+1,seed=`${level}-${topic}-${n}`,difficulty=difficultyFor(index,level),band=levelBand(level),variant=hash(seed)%12;
 if(f==='fraction'){
  ...
  if(variant%4===2){const x=Math.max(a,b),y=Math.min(a,b),correct=x-y;return q(`${level}-gen-${n}`,topic,`What is ${fraction(x,den)} − ${fraction(y,den)}? Give the numerator over ${den}.`,correct,`${x}−${y}=${correct}, with denominator ${den}.`,difficulty,'Subtract fractions with like denominators')}
 ...
 if(f==='algebra'){
  ...
  const correct=a*x;return q(`${level}-gen-${n}`,topic,`The expression ${a}x is evaluated at x=${x}. What is its value?`,correct,`Substitute x=${x}: ${a}×${x}=${correct}.`,difficulty,'Evaluate algebraic expressions')
 }
 if(f==='geometry'){
 ...
 if(f==='stats'){
  ...
  if(type===3){const sum=values.reduce((a,b)=>a+b,0),correct=sum/5;return q(`${level}-gen-${n}`,topic,`Five learners scored ${values.join(', ')} marks. What is their mean score?`,correct,`Add the scores to get ${sum}, then divide by 5 to get ${correct}.`,difficulty,'Find the mean of a data set')}
  const boys=rand(`${seed}y`,2,8),girls=rand(`${seed}n`,2,8);return q(`${level}-gen-${n}`,topic,`A class recorded ${boys} boys and ${girls} girls in a survey. How many boys were recorded?`,boys,`The number recorded for boys is ${boys}.`,difficulty,'Interpret simple frequency data')
 }
 if(f==='trig'){
  const type=variant%4,k=rand(seed,2,30);
  ...
  const hyp2=2*k*k;return q(`${level}-gen-${n}`,topic,`A right triangle has legs ${k} cm and ${k} cm. What is the square of its hypotenuse?`,hyp2,`By Pythagoras, c²=${k}²+${k}²=${hyp2}.`,difficulty,'Use Pythagoras with a right triangle')
 }
 if(f==='calculus'){
  ...
  if(variant%3===1){const correct=2*a*c;return q(`${level}-gen-${n}`,topic,`If y=${a}x², what is dy/dx when x=${c}?`,correct,`dy/dx=2${a}x. Substituting x=${c} gives ${correct}.`,difficulty,'Differentiate a quadratic function')}
  return q(`${level}-gen-${n}`,topic,`What is the derivative of ${a}x?`,a,`The derivative of ax is a.`,difficulty,'Use basic differentiation rules')
 }
 if(f==='financial'){
