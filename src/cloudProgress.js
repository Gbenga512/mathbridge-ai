...
const localSave=(userId,progress)=>{try{localStorage.setItem(`${KEY}:${userId}`,JSON.stringify({...progress,updatedAt:new Date().toISOString()}));return {ok:true,mode:'local-mvp'}}catch{return {ok:false,mode:'local-mvp'}}};
...
export async function signUpStudent({email,password,fullName,classLevel}){
 ...
 const {data,error}=await supabase.auth.signUp({email,password,options:{data:{role:'student',full_name:fullName,class_level:classLevel||null}}});
...
export async function saveAssessment(userId,assessment){if(!supabaseConfigured||!userId||userId==='local-user')return {ok:false,mode:'local-mvp'};const {error}=await supabase.from('assessments').insert({student_id:userId,...assessment});return {ok:!error,mode:'supabase',error};}
