...
let p=createJSS3Progress();p=recordJSS3TopicResult(p,unique[0],4,5);const s=getJSS3ProgressSummary(p,unique);if(s.masteredCount!==1||s.percentage!==3)errors.push('Mastery tracking failed');
if(errors.length){console.error(errors.join('\n'));process.exit(1)}console.log(`JSS 3 QA passed: ${unique.length} topics, ${jss3Questions.length} questions, mastery tracking OK.`);
