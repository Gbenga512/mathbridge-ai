...
-- Student accounts and learning records are student-owned.
-- School enrollment/subscription is a separate relationship and MUST NOT delete/reset student learning data.
...
create table if not exists public.schools (
 ...
 enabled_sections text[] not null default '{JSS}',
 created_at timestamptz not null default now()
);
...
begin
 ...
 values(new.id,coalesce(new.raw_user_meta_data->>'role','student'),coalesce(new.raw_user_meta_data->>'full_name','User'),new.raw_user_meta_data->>'class_level')
 on conflict(id) do nothing;
 if coalesce(new.raw_user_meta_data->>'role','student')='student' then
   insert into public.progress(student_id,term,week,mastered_weeks) values(new.id,'T1',1,'{}') on conflict(student_id) do nothing;
 end if;
 return new;
end; $$;
...
drop policy if exists "school subscriptions admin" on public.school_subscriptions;
...
create policy "school subscriptions admin" on public.school_subscriptions for select using (public.is_school_staff(school_id));
