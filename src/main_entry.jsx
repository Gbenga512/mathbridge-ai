...
function SSSShell(){const[view,setView]=React.useState('levels');const goBack=()=>window.location.reload();if(view==='results')return <main className="sss-shell"><button className="secondary" onClick={()=>setView('levels')}>← Back to SSS</button><ExamResultsDashboard/></main>;return <main className="sss-shell"><div className="sss-head"><div><p className="step">SENIOR SECONDARY SCHOOL</p><h1>MathBridge SSS Learning Centre</h1><p>Choose your class to continue learning.</p></div><div><button className="secondary" onClick={()=>setView('results')}>My Exam Performance</button><button className="secondary" onClick={goBack}>← Back to Curriculum</button></div></div><div className="sss-levels"><button className="sss-level" onClick={()=>window.__mathbridgeSSSRender('SSS1')}><strong>SSS 1</strong><span>Build your senior secondary foundation</span></button><button className="sss-level" onClick={()=>window.__mathbridgeSSSRender('SSS2')}><strong>SSS 2</strong><span>Develop deeper algebra, geometry and statistics</span></button><button className="sss-level" onClick={()=>window.__mathbridgeSSSRender('SSS3')}><strong>SSS 3</strong><span>Exam preparation, advanced topics and WAEC/NECO practice</span></button></div></main>}
