...
const familyFor=topic=>{const t=String(topic).toLowerCase();if(/set|venn/.test(t))return'sets';if(/fraction|decimal|percentage|sharing|ratio|proportion|rate|variation/.test(t))return'fraction';if(/statistics|data|probability|pictogram|chart|mean|median|mode|range/.test(t))return'stats';if(/trigonometric|trigonometry/.test(t))return'trig';if(/calculus|differentiation|integration/.test(t))return'calculus';if(/financial|money|profit|loss|discount|interest|applied/.test(t))return'financial';if(/coordinate|geometry|shape|angle|mensuration|perimeter|area|volume|symmetry|space|measurement|construction/.test(t))return'geometry';if(/matrix|matrices/.test(t))return'matrix';if(/permutation|combination/.test(t))return'counting';if(/vector/.test(t))return'vector';if(/sequence|series/.test(t))return'sequence';if(/logarithm|\blog\b|indices|index|surd|standard form/.test(t))return'indices';if(/function/.test(t))return'function';if(/algebra|equation|expression|quadratic|factor|formula/.test(t))return'algebra';return'number'};
...
const numberQuestion=(topic,level,n,seed,difficulty)=>{const band=levelBand(level),type=hash(`${seed}:type`)%8;const hi=band==='primaryLower'?50:band==='primaryUpper'?500:band==='jss'?5000:100000;const lead=pick(`${seed}:lead`,['Calculate','Work out','In this class exercise, find','A learner works out','Find the answer to']);
 if(type===0){const a=rand(`${seed}:a`,band==='primaryLower'?1:10,hi),b=rand(`${seed}:b`,1,hi);return q(`${level}-gen-${n}`,topic,`${lead}: ${a}+${b}.`,a+b,`${a}+${b}=${a+b}.`,difficulty,'Add whole numbers');}
 if(type===1){const a=rand(`${seed}:a`,1,hi),b=rand(`${seed}:b`,1,Math.max(1,a));return q(`${level}-gen-${n}`,topic,`${lead}: ${a}−${b}.`,a-b,`${a}−${b}=${a-b}.`,difficulty,'Subtract whole numbers');}
 if(type===2){const max=band==='primaryLower'?12:band==='primaryUpper'?25:50;const a=rand(`${seed}:a`,2,max),b=rand(`${seed}:b`,2,max);return q(`${level}-gen-${n}`,topic,`${lead}: ${a}×${b}.`,a*b,`${a}×${b}=${a*b}.`,difficulty,'Multiply whole numbers');}
 if(type===3){const b=rand(`${seed}:b`,2,25),a=rand(`${seed}:a`,2,25)*b;return q(`${level}-gen-${n}`,topic,`${lead}: ${a}÷${b}.`,a/b,`${a}÷${b}=${a/b}.`,difficulty,'Divide whole numbers');}
...
function generateUnique(level,topics,count,exam){const safe=Array.isArray(topics)&&topics.length?topics:['General Mathematics'];const out=[],seen=new Set();let candidate=0;const maxAttempts=Math.max(count*30,1000);while(out.length<count&&candidate<maxAttempts){const topic=safe[candidate%safe.length],item=make(topic,level,candidate),key=normalize(item.q);if(!seen.has(key)){seen.add(key);out.push(exam?{...item,id:`${exam}-gen-${out.length+1}`,exam,sourceType:`${exam}_STYLE_ORIGINAL`,topic:`${exam} • ${item.topic}`,objective:`${exam}-style: ${item.objective}`}:item)}candidate++}if(out.length<count)throw new Error(`Unable to generate ${count} unique ${exam||level} questions after ${candidate} attempts; expand generator variation space.`);return out;}
export function generateCurriculumBank(level,topics,count=2000){return generateUnique(level,topics,count,null)}
export function generateExamBank(exam,count=2000){const topics=['Number and Numeration','Algebra and Equations','Fractions, Percentages and Ratio','Geometry and Mensuration','Statistics and Probability','Trigonometry','Financial Mathematics','Sequences and Functions','Problem Solving'];return generateUnique(exam==='JAMB'?'SSS3':'SSS2',topics,count,exam)}
